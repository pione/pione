...
TestHelper.scope do |this|
  ...
  describe Pione::Command::PionePackage do
    ...
    it "should build PIONE package with the package name" do
      ...
      args = ["--build", "-o", output_location.path.to_s, this::P1.path.to_s]
    ...
    it "should build PIONE package with the tag" do
      output_location = Location[Temppath.mkdir]
      args = ["--build", "-o", output_location.path.to_s, this::P2.path.to_s]
      res = TestHelper::Command.succeed(@cmd, args)
    ...
    it "should build PIONE package with the editor" do
      output_location = Location[Temppath.mkdir]
      args = ["--build", "-o", output_location.path.to_s, this::P3.path.to_s]
      res = TestHelper::Command.succeed(@cmd, args)
    ...
    it "should build PIONE package with full name" do
      output_location = Location[Temppath.mkdir]
      args = ["--build", "-o", output_location.path.to_s, this::P4.path.to_s]
      res = TestHelper::Command.succeed(@cmd, args)
    ...
    it "should add package with name to database" do
      TestHelper::Command.succeed(@cmd, ["--add", this::P1.path.to_s])
    ...
    it "should add package with tag to database" do
      res = TestHelper::Command.succeed(@cmd, ["--add", this::P2.path.to_s])
      db = Package::Database.load
    ...
    it "should add package with editor to database" do
      res = TestHelper::Command.succeed(@cmd, ["--add", this::P3.path.to_s])
      db = Package::Database.load
    ...
    it "should add package with full name to database" do
      TestHelper::Command.succeed(@cmd, ["--add", this::P4.path.to_s])
      db = Package::Database.load
    ...
    it "should add tag alias" do
      TestHelper::Command.succeed(@cmd, ["--add", "--tag", "TEST", this::P4.path.to_s])
      db = Package::Database.load
